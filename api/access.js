...
async function refreshVotes(code){for(let stage=1;stage<=2;stage+=1){const rows=await Promise.all(Array.from({length:SHARDS},(_,i)=>cache().get(voteKey(code,stage,i))));await Promise.all(rows.map((value,i)=>value?cache().set(voteKey(code,stage,i),value,{ttl:ROOM_TTL}):Promise.resolve()));}}
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(action==='create'){
      ...
      let code='';for(let i=0;i<10;i+=1){const candidate=newCode();if(!await cache().get(roomKey(candidate))){code=candidate;break;}}
    ...
    if(action==='resume'){
      ...
      await cache().delete(failKey(code));room.lastActiveAt=Date.now();room.updatedAt=room.lastActiveAt;await Promise.all([saveRoom(room),refreshVotes(code)]);
    ...
    if(action==='setPin'){
      ...
      const pin=clean(body.pin,12);if(!validPin(pin))return res.status(400).json({error:'bad_pin'});
      const p=makePin(pin);room.pinSalt=p.salt;room.pinHash=p.hash;room.pinVersion=(room.pinVersion||0)+1;room.lastActiveAt=Date.now();room.updatedAt=room.lastActiveAt;await Promise.all([saveRoom(room),refreshVotes(code)]);
      return res.json({ok:true,pinConfigured:true,lastActiveAt:room.lastActiveAt});
    }
    if(action==='touch'){
      if(!sameToken(clean(body.teacherToken,120),room.teacherToken))return res.status(403).json({error:'teacher_auth_failed'});
      room.lastActiveAt=Date.now();room.updatedAt=room.lastActiveAt;await Promise.all([saveRoom(room),refreshVotes(code)]);return res.json({ok:true,lastActiveAt:room.lastActiveAt});
    }
